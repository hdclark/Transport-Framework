...
file_input    = "/tmp/Transport_Detector.process"
file_output   = "/tmp/Transport_Detector.plot"
numb_of_bins  = 512   --The upper limit on the number of bins.
heading       = " detector#    # of counts"
...
local pattern = "%s*([%E%.%+%-%e%d]+)%s+([%E%.%+%-%e%d]+)%s+([%E%.%+%-%e%d]+)%s+([%E%.%+%-%e%d]+)%s+([%E%.%+%-%e%d]+)%s+"

for n1, n2, n3, n4 in string.gfind(io.read("*all"), pattern) do
--        print("Found numbers " .. n2 .. " and " .. n3)
        n1 = tonumber(n2)
        n2 = tonumber(n3)
...
for i=0,(numb_of_bins-1) do
    ...
    if gnuplot then
        output__file:write( (i  ) .. " " ..       "0" .. " " .. "\n")
        output__file:write( (i  ) .. " " .. output[i] .. " " .. "\n")
        output__file:write( (i+1) .. " " .. output[i] .. " " .. "\n")
        output__file:write( (i+1) .. " " ..       "0" .. " " .. "\n")
    else
        output__file:write( (i) .. " " .. output[i] .. " " .. "\n")
    end
end
