...
file_output   = "/tmp/Transport_Photon_Angular_Distribution_theta.plot"
numb_of_x_bins  = 12    -- 12 energy bins.
numb_of_y_bins  = 50    -- 50 angular bins.
heading       = " Theta  Intensity@Energy1  Intensity@Energy2 Intensity@Energy3   ... "
gnuplot       = false --Prints extra points to illustrate bin size, etc. Useful for direct Gnuplot printing.
...
for n1, n2, n3 in string.gfind(io.read("*all"), pattern) do
        ...
        n1 = tonumber(n1)
        n2 = tonumber(n2)   --  <-----Note we only use the second number here. Switch to n3 if you want to see other angle.
...
output__file:write("#  -- Post analysis: The maximum theta value was" .. " " .. max_y .. " " .. "\n")
output__file:write("#  -- Post analysis: The minimum theta value was" .. " " .. min_y .. " " .. "\n")
        ...
        if( (x >= (min_x + i*x_bin_spacing)) and (x < (min_x + (i+1)*x_bin_spacing))) then
            ...
            for j=0,(numb_of_y_bins-1) do
                if( (y >= (min_y + j*y_bin_spacing)) and (y < (min_y + (j+1)*y_bin_spacing))) then
                    ...
                    output[i][j] = output[i][j] + 1 --y     --Sort the samples into their proper energy and angle bins.
...
for i=0,(numb_of_x_bins-1) do
    local this_bin_total = 0  --Normalize (over theta) for each energy bin.
    for j=0,(numb_of_y_bins-1) do
