//Coherent.cc - Photon interactions: Coherent (Rayleigh) scattering.
//
//Programming notes:
...
std::string INTERACTION_TYPE("COHERENT");
...
void scatter(std::unique_ptr<base_particle> A, const struct Functions &Loaded_Functions){
    ...
    //Coherent scatter involves no energy exchange between the photon and the electron. No particles are created
    // or destroyed - only the photon is deflected.
    //
    //For the purposes of the Phys 539 assignment, we assume the scattering angle is negligible and let the particle continue along its path.
    
    //Needed in this function: 
    // - A uniformly random orientation generator.
    // - Access to the memory sink function.


    if(A->get_type() != Particletype::Photon){
        FUNCERR("Coherent scattering only implemented for photons. Attempted to perform scatter event on particle of type " << A->get_type());
    }

/*
    //Assign the incoming photon a new, randomly-oriented orientation.
    A->set_relativistic_three_momentum3( Loaded_Functions.get_random_orientation() );
*/


    //Push the photon back into memory.
