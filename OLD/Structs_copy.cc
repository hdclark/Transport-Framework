...
//Methods - non-virtual.
unsigned char base_particle::get_type(void){ return type; }
double        base_particle::get_mass(void){ return mass; }
double        base_particle::get_charge(void){ return charge; }
...
//vec3<double> base_particle::get_momentum3(void){ return vec3<double>( (U.p).x*mass,(U.p).y*mass,(U.p).z*mass ); }
vec3<double> base_particle::get_momentum3(void){ return (U.p)*mass; }
void base_particle::set_momentum3(const vec3<double> &in){
    if(type == Particletype::Photon ) return;  // FIXME
    U.p = in; 
    U.p /= mass; 
    return; 
}
